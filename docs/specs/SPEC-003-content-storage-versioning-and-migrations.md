...
type DocumentVersionResponse = DocumentVersion & {
  resolveErrors?: Record<string, ReferenceResolveError | FileFieldResolveError>;
};
```
...
| GET    | `/api/v1/content/:documentId/versions/:version`         | session_or_api_key | `content:read`                                                                                             | required: `project_environment` | path `documentId`, `version`, optional `resolve`, optional `fileFields`                                                                                                                                             | `200` `{ data: DocumentVersionResponse }`                                                                                                      | `MISSING_TARGET_ROUTING` (`400`), `TARGET_ROUTING_MISMATCH` (`400`), `INVALID_QUERY_PARAM` (`400`), `INVALID_INPUT` (`400`), `UNAUTHORIZED` (`401`), `FORBIDDEN` (`403`), `NOT_FOUND` (`404`)                                                                                                                                                                                                                                         |
